...
const options = {
  ...
  proxy: [{
    context: ['/core/v5', '/storage/local'],
    target: ' http://10.231.50.34',
    onProxyReq(proxyReq, req, res) {
      proxyReq.setHeader('Cookie', 'token=vMdxaiUEaQSErZjguMJCUkvlHIvpkcTmMpnrlKQfPKTkiiqLNtKfYeEsClot');
      return '';
    }
  }]
};
...
portfinder.getPort((err, port) => {
  if(err) {
    ...
    if(process.env.npm_config_module &&  process.env.npm_config_module !== '') {
      ...
      modules = _.without(fs.readdirSync(path.join(__dirname, '../src/modules')), 'layout', 'common', 'login', 'manage', '.DS_Store').join(',');
    }

    let devModulesTip = `编译模块（${modules.split(',').length}）：${modules}`;
    ...
    // webpackConfig.plugins.push(errorsPlugin);
