  ...
  <div class="photo-add">
    <div class="photo-add-content">
      <div
        class="photo-upload">
        <meg-img-container
          :useful-frame="addBodyPhotoModel.rect"
          :img-src="addBodyPhotoModel.photo">
        </meg-img-container>
      </div>
      <div class="photo-add-form">
        <meg-form
          ...
          :model="addBodyPhotoModel">
          <div class="basic-info">
            <meg-form-item
              :label="$t('姓名')"
              prop="name">
              <meg-input
                v-model="addBodyPhotoModel.name"
            ...
            <meg-form-item
              ...
              prop="criminalNumber">
              <meg-input
                v-model="addBodyPhotoModel.criminalNumber"
                :placeholder="$t('请输入在逃编号')"></meg-input>
            </meg-form-item>
            <meg-form-item :label="$t('性别')">
              <meg-radio-group v-model="addBodyPhotoModel.gender">
                <meg-radio
            ...
            <meg-form-item
              v-if="addBodyPhotoModel.nation"
              :label="$t('民族')">
              <meg-select
                class="meg-select database-select"
                v-model="addBodyPhotoModel.nation.nationCode">
                <meg-option
            ...
            <meg-form-item :label="$t('出生日期')">
              <meg-date-picker
                ref="datePicker"
                v-model="addBodyPhotoModel.birthday"
                type="date"
                format="yyyy-MM-dd"
            ...
            <meg-form-item
              ...
              <meg-select
                ...
                v-model="addBodyPhotoModel.addrCode"
                filterable
                clearable
                remote
            ...
            <meg-form-item
              ...
              <meg-input
                ...
                resize="none"
                rows="3"
                v-model="addBodyPhotoModel.cmnt">
              </meg-input>
            </meg-form-item>
          </div>
          <div class="other-info">
            <div class="identity-content">
              ...
              <meg-form-item
                ...
                v-for="(identity, index) in addBodyPhotoModel.identitys"
                :key="index"
                ...
                <meg-button
                  v-show="sameSfzBodyPhotos.index === index && sameSfzBodyPhotos.photos.length > 0"
                  type="text"
                  class="same-sfz-btn"
                ...
                <span class="action-btn">
                  <span
                    :style="{ visibility: addBodyPhotoModel.identitys.length === 1? 'hidden' : '' }"
                  ...
                  <span
                    v-show="identityOptions.length !==addBodyPhotoModel.identitys.length && index === addBodyPhotoModel.identitys.length-1"
                    @click="addIdentity">
                    <meg-icon
                      class="add-btn"
              ...
              <div v-if="isSFZshow">
                <meg-form-item
                  ...
                  <meg-input
                    v-model="addBodyPhotoModel.identityExt.sfz.issueOrgan"
                ...
                <meg-form-item
                  ...
                  <meg-input
                    v-model="addBodyPhotoModel.identityExt.sfz.effectivePeriod"
            ...
            <meg-form-item
              ...
              <meg-input
                v-model="addBodyPhotoModel.identityExt.sfz.homeAddress"
            ...
            <div>
              <meg-form-item
                ...
                <meg-input
                  ...
                  resize="none"
                  rows="3"
                  v-model="addBodyPhotoModel.monitorReason">
                </meg-input>
              </meg-form-item>
              <meg-form-item
                :label="$t('报警联系人')"
                ...
                prop="contact">
                <meg-input
                  v-model="addBodyPhotoModel.contact"
...
export default {
  ...
  beforeMount() {
    ...
    if (Object.keys(this.addPhotoData).length !== 0) {
      ...
      Object.assign(this.addBodyPhotoModel, this.addPhotoData);
      if (this.addBodyPhotoModel.identitys.length === 0) {
        this.initIdentities();
      } else {
        this.addBodyPhotoModel.identitys.forEach((item) => {
  ...
  methods: {
    onIdentityClear(identity) {
      if (identity.code === 'sfz') {
        this.sameSfzBodyPhotos.index = 0;
        this.sameSfzBodyPhotos.photos = [];
      }
    },
    initIdentities() {
      // 默认进来没有证件号显示
      ...
      this.addBodyPhotoModel.identitys.push(defaultOption);
    ...
    onBodyPhotoAddConfirm() {
      this.$refs.addBodyPhotoForm.validate((valid) => {
        if (valid && this.addBodyPhotoModel.photo !== '') {
          const addr = _.find(this.matchedOriginOptions, { 'code': this.addBodyPhotoModel.addrCode });
          this.addBodyPhotoModel.addrMsg = addr ? addr.value : '';
          for (let i = 0; i < this.nationOptions.length; i += 1) {
            if (this.nationOptions[i].code === this.addBodyPhotoModel.nation.nationCode) {
              this.addBodyPhotoModel.nationCode = this.nationOptions[i].code;
              this.addBodyPhotoModel.nationMsg = this.nationOptions[i].value;
            }
          }
          this.addBodyPhotoModel.birthday = this.addBodyPhotoModel.birthday ? moment(new Date(this.addBodyPhotoModel.birthday)).format('YYYY-MM-DD') : '';
          this.addBodyPhotoModel.st = this.addBodyPhotoModel.st ? moment(new Date(this.addBodyPhotoModel.st)).format('YYYY-MM-DD') : '';
          this.$emit('body-photo-add-confirm', this.addBodyPhotoModel);
        }
      });
    },
    /**
    ...
    onBodyPhotoAddCancel() {
      this.$emit('body-photo-add-cancel');
    },
    ...
    sameSfz(value, index) {
      this.$emit('body-sfz-input-change', { value, index });
    },
    /**
     * 增加证件类型
     */
    addIdentity() {
      ...
      this.addBodyPhotoModel.identitys.push({
        options: newOptions,
     ...
     */
    deIdentity(index) {
      this.addBodyPhotoModel.identitys.splice(index, 1);
