    ...
    <meg-collapse
      ...
      <meg-collapse-item
        ...
        <template slot="title">
          ...
          <span class="record-num"> {{ $t('出现{count}次', { count: item.count }) }} </span>
        </template>
        <div
          class="collapse-block"
          v-loading="loading && index===activeName">
          <!-- <meg-scrollbar> -->
          <meg-card-frame :card-min-width="262">
            <div
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.activeName = 0;
      this.lastName = 0;
      this.updatePageData(this.activeName);
    },
    setActiveList(list) {
    ...
    updatePageData(index) {
      ...
      this.activeLists[index].list.push(...temp);
    },
    /**
     * 点击卡片时提交点击事件
