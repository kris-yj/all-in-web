            ...
            <common-collapse
              ...
              <template v-slot="{item,index}">
                <alarm-card
                  ...
                  @store-change="(type,data) => onStoreChange(type,data,index)"
...
import CreateDeployType from './create-deploy-type';
...
export default {
  ...
  data() {
    return {
      ...
      isShowCreateDeployType: false,
      isSaving: false,
      categoryNum: 0, // 每个手风琴下总数
      enableCreate: false, // 是否具有综合管理权限
    };
  },
  computed: {
    isShowLoadMore() {
    ...
    getTaskDetail() {
      ...
      SurveillanceTaskSvr.getTaskDetail({ id: this.taskId }).then((response) => {
        if (response.code === 0) {
          ...
          const { cameraValues, cameraIds } = getCameraInfo(taskDetailData.cameraList);
          taskDetailData.cameraIds = cameraIds;
          // 将cameraList删除避免挂载到data上造成崩溃
          delete taskDetailData.cameraList;
          leftInfo.camerasStr = cameraValues.join(',');
          leftInfo.enableEdit = !taskDetailData.readOnly && !taskDetailData.applyReadOnly && !taskDetailData.dualNetSync && this.enableCreate;
          leftInfo.alarmType = taskDetailData.alarmType;
          leftInfo.cameraCount = taskDetailData.cameraCount;
