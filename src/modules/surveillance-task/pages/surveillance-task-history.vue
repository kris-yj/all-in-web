...
export default {
  ...
  methods: {
     ...
     */
    getCameraTree() {
      SurveillanceTaskSvr.getCameraTree().then((res) => {
        if (res.code === 0) {
     ...
     */
    getAllAlbumList() {
      this.getCameraAlbum({ history: true }, (data) => {
    ...
    getCameraAlbum(data = {}, fn = () => {}) {
      SurveillanceTaskSvr.getAlbumListSpecial(data).then((res) => {
        if (res.code === 0) {
          fn(res.data);
        }
      });
    ...
    getDetailData(task, type) {
      ...
      SurveillanceTaskSvr.getTaskDetail({ id: task.id }).then((response) => {
        if (response.code === 0) {
          ...
          vm.defaultFormData = taskDetailData;
    ...
    toTaskDetail(data) {
      this.$router.push({ name: 'surveillance-task-history-detail', params: { taskId: data.id } });
    },
