...
export default {
  ...
  methods: {
    ...
    handleOriginToStamp(timeArr) {
      ...
      return arr.map((ele) => {
        ele.startMinute = new Date(ele.startMinute).getTime() - moment().startOf('day').toDate().getTime();
        ele.endMinute = new Date(ele.endMinute).getTime() - moment().startOf('day').toDate().getTime();
        return ele;
      });
    },
  }
