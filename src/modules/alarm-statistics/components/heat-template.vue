<template>
  <div
    ...
    class="template">
    <number-list
      :data="heatData.warningStatistic"
      :options="infoOption"></number-list>
    <div class="alarm-situation-map">
      <heatmap-map
        ref="map"
...
export default {
    ...
    return {
      infoOption: [
        ...
        },
        {
          icon: 'card_face',
          label: this.$t('人脸图片报警'),
          key: 'faceWarningCount'
        },
        {
          icon: 'person',
          label: this.$t('人体图片报警'),
          key: 'bodyWarningCount'
        },
        {
          icon: 'car',
          label: this.$t('机动车报警'),
        ...
        },
        {
          ...
          label: this.$t('非机动车报警'),
          key: 'nonMotorWarningCount'
        },
        {
          icon: 'property-search_menu',
          label: this.$t('检测报警'),
          key: 'detectWarningCount'
        }
      ],
    };
  },
  computed: {
     ...
     */
    heatmapData() {
      if (!this.heatData.records) return [];
      const list = [];
      console.log(this.heatData.records);
  ...
  methods: {
    ...
    async generatorImage() {
      ...
      const heatMapImg = await chartOption.generatorImage(this.$refs.template);
