        ...
        <div class="plan-left">
          <div class="vsr-dialog-content">
            ...
            <div class="wrap input-width">
              ...
              <meg-input
                v-model="detailData.speed"></meg-input>
            </div>
            <div class="wrap">
              <span class="label">{{ $t('优先级') }}</span>
              <meg-radio-group
                v-model="detailData.priority">
                <meg-radio
                  :label="1">
                  一般任务
                </meg-radio>
...
export default {
      ...
      const mapText = Object.keys(albums).map((key) => {
        ...
        if (data.length) {
          const val = data.map((item) => {
            ...
            const thres = item.threshold ? `, 阈值:${item.threshold}` : '';
  ...
  methods: {
    ...
    async saveEdit() {
      ...
      const { speed, taskName } = this.detailData;
      if (!taskName) {
      ...
      if (!this.checkValue(speed)) {
        ...
        return this.$message.error({
          message: `请输入0-${totalVideoPower}的解析倍速`,
          showClose: true
        });
      }
