  ...
  <page-layout>
    <div class="region__lists">
      ...
      <task-dialog
        ...
        :album-tree-selector-data="albumTreeSelectorData"
...
export default {
  ...
  computed: {
    analysisManage() {
      return this.$store.getters.modulePermissonsByAlias('album-region', 'analysisManage');
    },
    platformType() {
      //  1是否公安网，0为视频网
  ...
  created() {
    ...
    AlbumRegionSrv.cameraList(allCamera).then((res) => {
      if (res.code === 0) {
        this.cameraList = res.data;
        this.$refs.taskDetail.setTreeData(res.data);
      }
    });
    this.init();
    this.threshold();
  ...
  methods: {
    ...
    threshold() {
      AlbumRegionSrv.threshold().then((res) => {
        if (res.code === 0) {
    ...
    getTreeData() {
      ...
      return AlbumRegionSrv.getAlbum(params).then((response) => {
        if (response.code === 0) {
          const albumListRoot = {
            flag: 'root',
            name: this.$t('全部'),
    ...
    terminateTask(taskId) {
      ...
      this.$confirm(this.$t('您确定要终止任务吗，终止后将停止运行'), this.$t('终止任务'), {
        ...
        AlbumRegionSrv.terminateTask(taskId).then((res) => {
          if (res.code === 0) {
            vm.$message.success(this.$t('终止成功'));
            vm.init();
          } else {
     ...
     */
    restartTask(taskId) {
      AlbumRegionSrv.restartTask(taskId).then((res) => {
        if (res.code === 0) {
     ...
     */
    taskConfirm(task) {
      AlbumRegionSrv.addTask(task).then((res) => {
        if (res.code === 0) {
    ...
    /**
       ...
       */
    getStatistics() {
      AlbumRegionSrv.regionStatistics().then((response) => {
        if (response.code === 0) {
    ...
    /**
       ...
       */
    getList() {
      AlbumRegionSrv.getRegionList(this.taskListsParam).then((response) => {
        if (response.code === 0) {
     ...
     */
    deleteTask(id) {
      AlbumRegionSrv.deleteTask(id).then((res) => {
        if (res.code === 0) {
    ...
    async copyTask(task) {
      ...
      this.taskType = 'copy';
      AlbumRegionSrv.jobDetail(task.jobId).then((res) => {
        if (res.code === 0) {
          this.taskDialogVisible = true;
          const albumIds = res.data.albumVos.reduce((arc, item) => [...arc, item.id], []);
          const cameraIds = [];
          res.data.region.cameraVos.forEach((it) => {
            cameraIds.push(it.id);
          });
          res.data.region.cameraIds = cameraIds;
          this.taskFormData = {
            jobName: `${res.data.jobName}-${this.$t('副本')}`,
            threshold: res.data.threshold,
            region: res.data.region,
            albumIds,
            compareMode: res.data.compareMode,
            jobType: '1'
          };
          this.$nextTick(() => {
            this.$refs.taskDialog.copyDate();
          });
        } else {
          this.$message.error(res.msg);
        }
